...
├── week10/                            ← Phase 6: Spark Streaming (Part 1)  ✅ DONE
├── week11/                            ← Phase 6: Spark Streaming (Part 2)  ✅ DONE
├── week12/                            ← Phase 7: Advanced — Delta Lake & Cluster Config  ✅ DONE
├── week13/                            ← Phase 7: Cloud Deep Dive & Lakehouse  ✅ DONE
├── week14/                            ← Phase 7: Orchestration & AWS Integration  ✅ DONE
├── week15/                            ← Phase 7: Testing, CI/CD & Data Quality  ✅ DONE
└── week16/                            ← Phase 7: Capstone Project  ✅ DONE
```
...
### Phase 3 — DataFrames & Spark SQL → Weeks 3-5

| Week | Status | Topics |
|------|--------|--------|
| **Week 3** | ✅ Done | DataFrame vs RDD, Schema (StructType, StructField), Reading CSV / JSON / Parquet / ORC, select / filter / withColumn / groupBy / agg |
| **Week 4** | ✅ Done | All join types (inner / left / right / full / semi / anti), Window Functions (rank, dense_rank, lead, lag, row_number) |
| **Week 5** | ✅ Done | UDFs, built-in functions, Spark SQL (createOrReplaceTempView, CTEs, subqueries) |

### Phase 4 — Performance & Optimization → Weeks 6-7

| Week | Status | Topics |
|------|--------|--------|
| **Week 6** | ✅ Done | Partitioning (repartition vs coalesce, skew, salting), Caching & Persistence (cache vs persist, all storage levels, unpersist), Shuffle optimization, spark.sql.shuffle.partitions, bucket joins |
| **Week 7** | ✅ Done | AQE (Adaptive Query Execution), Spark UI deep dive (spill/skew detection), explain() (physical vs logical plan), Catalyst Optimizer, Tungsten Engine, Memory management + OOM debugging |

### Phase 5 — PySpark ETL Engineering → Weeks 8-9

| Week | Status | Topics |
|------|--------|--------|
| **Week 8** | ✅ Done | SparkSession config, reading from S3/HDFS, writing with partitionBy(), schema enforcement, bad record handling, Medallion Architecture (Bronze → Silver → Gold) |
| **Week 9** | ✅ Done | Idempotent pipelines, incremental vs full loads, watermarking, pipeline architecture (error handling, dead letter queues, logging, checkpointing), Delta Lake (ACID, time travel, MERGE INTO, schema evolution, OPTIMIZE, ZORDER, Vacuum) |

### Phase 6 — Spark Streaming → Weeks 10-11

| Week | Status | Topics |
|------|--------|--------|
| **Week 10** | ✅ Done | Structured Streaming (micro-batch vs continuous, trigger options), input sources & output sinks, stateless vs stateful, watermarking for late data |
| **Week 11** | ✅ Done | Window operations (tumbling, sliding, session), Kafka + Spark (reading/writing, exactly-once semantics) |

### Phase 7 — Advanced & Master Level → Weeks 12-16

| Week | Status | Topics |
|------|--------|--------|
| **Week 12** | ✅ Done | Delta Lake deep dive (MERGE INTO, ZORDER, Vacuum, change data feed), cluster config (executor count, cores, memory, dynamic allocation) |
| **Week 13** | ✅ Done | Delta Lake advanced, Databricks (Unity Catalog, DLT, Auto Loader), EMR deep dive, Glue advanced, Lakehouse architecture |
| **Week 14** | ✅ Done | Apache Airflow (DAGs, Sensors, XCom, TaskGroups), Spark operators on EMR, S3 pipeline patterns, monitoring & alerting, end-to-end AWS pipeline |
| **Week 15** | ✅ Done | Unit testing PySpark (pytest, chispa), Data Quality framework (5 dimensions, dead-letter), Great Expectations, CI/CD with GitHub Actions, production best practices |
| **Week 16** | ✅ Done | Capstone: ShopStream e-commerce pipeline — system design, Bronze/Silver/Gold/Streaming, Airflow DAG, CI/CD, final interview prep |
